...
The published campaign runs the same exhaustive miner at six thresholds. Re-executing it reproduces the published counts exactly:
...
That single 22-item pattern is shared by exactly eight proteins, and each of the eight has exactly those 22 items and nothing else, which is why the enumeration cannot go deeper. Its members read like the résumé of a neuronal antiviral RNA helicase: the two DEAD/DEAH-box helicase domains (Pfam PF00270 and PF00271), ATP binding, RNA and DNA helicase activity, single- and double-stranded nucleic-acid binding, innate immune response, defense response to virus, cellular response to heat, and a spread of locations from cytosol and nucleus to mitochondrion, axon, dendrite, and nuclear speck, together with the medium-confidence structure bin. None of its 19 GO terms is an ancestor of another, so the depth is not an artifact of GO's hierarchy.
...
- A 12-item bacterial cell-wall synthase signature (transpeptidase PF00905 with transglycosylase PF00912) in about ten and a half thousand proteins.

## Is this just what any big table would produce?

A permutation test says no. Shuffling the annotations among proteins while preserving how many annotations each protein has, then re-mining at the 769-protein threshold, gives about 171,000 frequent itemsets per shuffle against 475,865 in the real data. More telling than the totals is the depth: shuffled data never produce a single itemset beyond *K* = 6, whereas the real data contain 88,745 itemsets of size seven or more. The deep combinations are properties of biology and of how curators annotate it, not of the table's marginals.

## What it costs

On the GPU the paper used (one NVIDIA H100), the deepest run takes 7.3 minutes. The re-execution used two consumer RTX 3090 cards: the same run takes 16 minutes of mining (19 minutes including the matrix build) on one of them with the single-GPU kernels, or 44 minutes with the two-GPU row-split path that streams every level to disk. The whole six-threshold campaign takes 38 minutes on a 3090. Feature extraction, the unglamorous part, reads a 150 GiB compressed UniProt flat file and the AlphaFold metadata table; it took 63 minutes in the original run.
...
Every number above was re-derived on 2 September 2026 from a fresh download of the AlphaFold DB metadata and UniProt TrEMBL, a fresh extraction, and fresh mining runs, and matched the published counts exactly. Two caveats surfaced along the way. The published paper states UniProt release 2025_01, but the record counts in the original logs identify release 2026_01, and only that release reproduces the results. And the paper's figures for a *streaming* variant of the miner (an approximate, chunked SON-style pass) could not be reproduced: the code as released is exact at every threshold tried, so those approximate-mode numbers should be read as historical.
